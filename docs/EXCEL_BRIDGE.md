...
工具缓存以账号、会话、凭证指纹隔离。账号模式 Compose 已启用 SQLite
持久化缓存（`EXCEL_TOOL_CACHE_PATH`），容器重建不再清空原生工具记录，
也不再受全局 4096 条内存淘汰限制。默认保留最近 7 天活跃的完整会话，
可用正整数 `EXCEL_TOOL_CACHE_TTL_SECONDS` 调整。删除持久卷仍会丢失记录。
缓存包含原生工具参数，应限制文件权限并按敏感业务数据管理备份。

需提供稳定 `prompt_cache_key` 或会话标识，并保持续接账号一致。
凭证轮换、账号/会话切换、空闲超期或升级前已丢失的历史仍返回 409；
不会跨账号查回或伪造原生调用。日志 `excel_history_miss` 区分失效原因，
仅记录账号 ID 和会话/调用哈希，不记录 Token、调用参数或完整会话标识。
未设置缓存路径时仍使用旧内存模式并在启动时警告。不要让不同主机上的
bridge 使用各自独立的缓存轮询同一工具会话，也不要将 SQLite 放在网络文件系统。
