...
func TestGuardCodexTurnStateEchoStripsCrossAccountEcho(t *testing.T) {
	...
	relayCodexTurnStateResponseHeader(c, affinityKey, minter, upstream, "http://sticky.example:9000")
	if got := codexTurnStateProxyForAffinity(affinityKey); got != "http://sticky.example:9000" {
		t.Fatalf("provenance egress = %q, want the relaying attempt's bound egress", got)
	}
